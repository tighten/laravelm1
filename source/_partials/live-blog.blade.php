...
released but not yet supported version.
        ...
        <li><span>Friday 10:37AM</span>Got taken away by my actual job. Back, that threw another error: memcached support requires ZLIB. <code>arm brew install zlib</code>.. another meeting...</li>
        <li><span>Friday 1:23PM</span>Sorry, day full of meetings. Hoping to finish memecached today, then I'll be back on tonight after kids' bed time trying to figure out more of them.</li>
        <li>Ran it during a meeting, see deprecation warning for <code>sasl_done</code> but looks like that's just a warning for now. HEY! Installed correctly to <code>/usr/local/Cellar/php/7.4.12/pecl/20190902/memcached.so</code>, and they already enabled it in <code>php.ini</code> for me.. let's test it! OK, class memcached not found. I'm gonna have to test things, also maybe restart php... nope that didn't do it. Will need to take some time, this may very well be pebkac, not m1, I remember Memcached and pecl are a pain some times. Gotta run get the kids though.</li>
        <li><span>Friday 9:54PM</span>Checked php.ini and <code>extension="memcached.so"</code> is up at the top... <code>php -i</code> indicates memcached support is enabled and libmemcached is connected.. and in Tinker, if I run <code>$m = new Memcached</code>, while it throws an error because no servers are defined, the class exists. So is this a Valet issue? Maybe that old Sock problem where Valet is hitting the wrong PHP?</li>
    </ul>
