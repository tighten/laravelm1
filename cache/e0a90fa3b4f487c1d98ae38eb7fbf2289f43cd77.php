<?php $__env->startSection('body'); ?>
<section class="container max-w-6xl mx-auto px-6 py-10 md:py-12">
    <div class="flex flex-col-reverse mb-8 lg:flex-row lg:mb-12">
        <div class="mt-8">
            <h1 ><?php echo e($page->siteName); ?></h1>

            <h2 class="font-light mt-4"><?php echo e($page->siteDescription); ?></h2>

            <p>Source here, in case you want to make PRs: <a href="https://github.com/tighten/laravelm1">github.com/tighten/laravelm1</a></p>
            <p>Another resource here: <a href="https://roaringapps.com/collections/list-of-apple-silicon-native-apps">https://roaringapps.com/collections/list-of-apple-silicon-native-apps</a></p>
        </div>
    </div>

    <div>
        <h4>Legend</h4>

        <table class="border border-gray-400">
            <tr>
                <td class="py-1 pl-3">
        <?php
            $colors = [
                "unsure" => "bg-gray-400",
                "no" => "bg-red-500",
                "yes" => "bg-green-500",
                "rosetta" => "bg-blue-500",
            ];
        ?>
        <div class="<?php echo $colors['unsure']; ?> inline-block w-3 h-3 rounded mr-2"></div></td>
                <td class="py-1 pr-3 pl-3">Haven't tested yet</td>
            </tr>
            <tr>
                <td class="py-1 pl-3">
        <?php
            $colors = [
                "unsure" => "bg-gray-400",
                "no" => "bg-red-500",
                "yes" => "bg-green-500",
                "rosetta" => "bg-blue-500",
            ];
        ?>
        <div class="<?php echo $colors['no']; ?> inline-block w-3 h-3 rounded mr-2"></div></td>
                <td class="py-1 pr-3 pl-3">Doesn't work as of testing</td>
            </tr>
            <tr>
                <td class="py-1 pl-3">
        <?php
            $colors = [
                "unsure" => "bg-gray-400",
                "no" => "bg-red-500",
                "yes" => "bg-green-500",
                "rosetta" => "bg-blue-500",
            ];
        ?>
        <div class="<?php echo $colors['rosetta']; ?> inline-block w-3 h-3 rounded mr-2"></div></td>
                <td class="py-1 pr-3 pl-3">Works, through Rosetta 2</td>
            </tr>
            <tr>
                <td class="py-1 pl-3">
        <?php
            $colors = [
                "unsure" => "bg-gray-400",
                "no" => "bg-red-500",
                "yes" => "bg-green-500",
                "rosetta" => "bg-blue-500",
            ];
        ?>
        <div class="<?php echo $colors['yes']; ?> inline-block w-3 h-3 rounded mr-2"></div></td>
                <td class="py-1 pr-3 pl-3">Works natively</td>
            </tr>
        </table>
    </div>

    <hr class="block my-8 border lg:hidden">

    <?php echo $__env->make('_partials.status', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php echo $__env->make('_partials.live-blog', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
</section>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('_layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/mattstauffer/Sites/laravelm1/source/index.blade.php ENDPATH**/ ?>